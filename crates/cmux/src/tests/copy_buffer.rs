...
/// A wide glyph occupies two cells but is one character in the row text, so a
/// range counted in cells would cut a CJK line in the wrong place.
#[test]
fn a_range_over_wide_glyphs_keeps_each_one_whole() {
    let b = CopyBuffer::new(vec![Row::new("日本語".to_string(), false)], (1, 20));
    assert_eq!(b.text_range((0, 0), (0, 2)), "日本語");
    assert_eq!(b.text_range((0, 1), (0, 1)), "本");
}

/// The opposite of every range test above: a region holding nothing but
/// blanks has to come back empty rather than as a run of spaces.
#[test]
fn a_range_over_blank_rows_is_empty() {
    let blank = vec![Row::new("   ".to_string(), false); 3];
